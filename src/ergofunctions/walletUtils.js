...
export async function getWalletAddresses(setup = false) {
    if(setup) {
        ...
        if (res && res !== 'denied') {
            try {
                return (await ergo.get_used_addresses()).concat(await ergo.get_unused_addresses())
            }
            catch {
                return "error"
            }
...
export async function getTokens() {
    ...
    try {
        const balances = await Promise.allSettled(addresses.map((address) => getBalance(address)));
        ...
        for (let i = 0; i < balances.length; i++) {
            if (balances[i].status === "fulfilled") {
                ...
                console.error(`Error getting balance for address ${addresses[i]}:`, balances[i].reason);
            }
        }
    } catch {
