...
export async function make_offer(nftId, price, buyBox){
    ...
    const r6 = listedBox ? listedBox.additionalRegisters.R6  : await getEncodedBoxSer(await boxById(nft_id));
    let publicKeyResponse = await axios.get(`${nodeUrl}/utils/addressToRaw/`+offeree).catch((err) => {
    ...
    const changeBox = {
        ...
        ergoTree: wasm.Address.from_mainnet_str(offeree).to_ergo_tree().to_base16_bytes(),
