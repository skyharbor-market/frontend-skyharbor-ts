...
import {
    showMsg,
    isWalletSaved,
    signTx, getListedBox, getWalletAddress
} from "../helpers";
import {
    txFee,
    supportedCurrencies,
...
export async function relist_NFT(relistBox, list_price, currencyIndex = 0) {
    ...
    const relistedBox = {
        ...
        assets: [
            {'tokenId': listedBox.assets[0].tokenId, 'amount': listedBox.assets[0].amount}
        ],
    ...
    const changeBox = {
        ...
        ergoTree: wasm.Address.from_mainnet_str(relister).to_ergo_tree().to_base16_bytes(),
