...
import {
    txFee,
    CHANGE_BOX_ASSET_LIMIT, bulkMintNodeAddress
} from "../consts";
...
export async function bulk_mint(hashes, size, royaltyPercent, receiveAddress, ergDollars){
    ...
    let hash
    // generate p2s
    const receiveAdd = binary_to_base58(fromHexString(blake.blake2bHex(fromHexString( new Address(receiveAddress).ergoTree), null, 32)));
    const royaltyAdd = binary_to_base58(fromHexString(blake.blake2bHex(fromHexString( new Address(minter).ergoTree), null, 32)));
    let script = `
    {
    ...
    const payServiceFee = {
        ...
        ergoTree: wasm.Address.from_mainnet_str(bulkMintNodeAddress).to_ergo_tree().to_base16_bytes(),
        assets: [],
        creationHeight: blockHeight.height,
        additionalRegisters: {}
    }
