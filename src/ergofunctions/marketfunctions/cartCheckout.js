...
import {
    txFee,
    CHANGE_BOX_ASSET_LIMIT, cartCheckoutAddress, babyTxFee, MIN_NERG_BOX_VALUE
} from "../consts";
...
export async function cart_checkout(cart) {
    ...
    const requiredErg = (totalCost + babyTxFee * cart.length + MIN_NERG_BOX_VALUE + txFee)
    ...
    let tokensTuple = [{nft_id: cart[0].token_id, price: cart[0].nerg_sale_value}]
    ...
    const cartBox = {
        value: (totalCost + babyTxFee * cart.length + MIN_NERG_BOX_VALUE).toString(),
        ergoTree: wasm.Address.from_mainnet_str(cartCheckoutAddress).to_ergo_tree().to_base16_bytes(),
        assets: [],
