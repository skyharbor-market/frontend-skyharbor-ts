import {
    showMsg,
    isWalletSaved,
    signTx, getOracleBox, getSigUsdBox
} from "../helpers";
import {
    txFee,
    supportedCurrencies,
    CHANGE_BOX_ASSET_LIMIT, babyTxFee, sigUsdAddress
} from "../consts";
...
export async function swapSigUSDtoERG(inputAmount, userAmountIn, skyHarborTaxIn) {
    ...
    const sigUsdBoxOutput = {
        ...
        ergoTree: wasm.Address.from_mainnet_str(sigUsdAddress).to_ergo_tree().to_base16_bytes(),
        assets: [
            {
        ...
        additionalRegisters: {
            "R4": await encodeNum((parseInt(sigUsdBox.additionalRegisters.R4.renderedValue) - parseInt(inputAmount)).toString()),
            "R5": await encodeNum((parseInt(sigUsdBox.additionalRegisters.R5.renderedValue)).toString())
        }
    }
    ...
    const oracleBoxObj = {
        ...
        assets: [{tokenId: oracleBox.assets[0].tokenId, amount: oracleBox.assets[0].amount}],
