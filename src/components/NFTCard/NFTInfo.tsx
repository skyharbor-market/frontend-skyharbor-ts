...
const NFTInfo = ({
  ...
  const currencyObject =
    SupportedCurrenciesV2[token.currency] || SupportedCurrenciesV2.erg;
  const nftPrice = (
    Math.round(
      longToCurrency(parseInt(token.nerg_sale_value), currencyObject.decimal) *
        1000
    ) / 1000
  ...
  return (
    <div
      key={`${token.token_id}-info`}
      className="w-full text-gray-900 dark:text-gray-200 bg-white dark:bg-gray-800"
    >
      <div
      ...
      <div className="flex flex-col md:flex-row md:space-x-8">
        ...
        <div className="w-full md:w-2/3 flex flex-col justify-between">
          <div>
            <h2 className="text-2xl font-semibold mb-0 text-gray-900 dark:text-gray-100">
              {token.nft_name}
            </h2>
            <div
              className="flex items-center mb-1 cursor-pointer hover:opacity-60 transition-opacity"
              onClick={gotoCollection}
            >
              <span
                className={`text-lg font-semibold ${
                  token.verified_collection
              ...
              >
                {token.verified_collection
                  ...
                  : `Unverified (${friendlyAddress(token.collection_name, 4)})`}
              </span>
              {token.verified_collection && (
                <CheckCircleIcon className="w-5 h-5 ml-1 text-blue-500 dark:text-blue-400" />
              )}
            </div>
            <p className="mb-0">
              <span className="font-semibold">Token ID: </span>
              <span
                className="text-blue-500 dark:text-blue-400 cursor-pointer hover:opacity-60 transition-opacity"
            ...
            <Link href={`/token/${token.token_id}`}>
              <a className="text-blue-500 dark:text-blue-400 hover:underline hover:opacity-60 transition-opacity mb-2 inline-block">
                View Token Page
              </a>
            </Link>
            <p className="mb-2">
            ...
            <div className="mb-4">
              <div
                className="flex items-center space-x-2 cursor-pointer hover:opacity-60 transition-opacity"
              ...
              {parseInt(token.total_existing) > 1 && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
                  {parseInt(token.token_amount).toLocaleString("en-US")} out of{" "}
                  {parseInt(token.total_existing).toLocaleString("en-US")}{" "}
                  minted
                </p>
              )}
          ...
          <div>
            <div className="flex justify-end items-baseline space-x-2 mb-4">
              <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                {nftPrice} {currencyObject.displayName}
              </span>
              {ergPrice && currencyObject.name === "erg" && (
                <span className="text-sm text-gray-600 dark:text-gray-300">
                  ...
                  {(
                    Math.round(
                      longToCurrency(token.nerg_sale_value) * ergPrice * 100
                    ) / 100
                  ).toLocaleString("en-US")}{" "}
                  USD)
                </span>
              ...
              {artistIsSeller && (
                <FaPaintBrush
                  ...
                  title="This NFT is being sold by the creator."
                />
              )}
            </div>
            {!noBuy && (
              <>
                <BuyNFTButton
                  ...
                  // ownedNFT={ownedNFT}
                  buyButton={
                    <Button className="w-full hover:opacity-60 bg-blue-500 text-white dark:bg-blue-600 dark:text-gray-100">
                      Buy
                    </Button>
                  }
                  editButton={
                  ...
                  loadingButton={
                    <div className="animate-pulse">
                      <Button
                        disabled
                        className="w-full bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400"
                      >
                        Loading...
                      </Button>
                    </div>
                ...
                />
                <p className="text-right mt-3 text-sm text-gray-500 dark:text-gray-400">
                  Listed {msToTime(diffTime)} ago
                </p>
              </>
            )}
          </div>
