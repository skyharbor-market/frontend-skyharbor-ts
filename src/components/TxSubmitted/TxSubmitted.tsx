...
export default function TxSubmitted({txId, box}) {
    ...
    function copyErgoPay() {
        navigator.clipboard.writeText(`ergopay://${skyHarborApiRoot}/api/ergopay/getTx/${txId}/${userAddress}`).then(()=> toast({
            title: "Copied",
            variant: "subtle",
            ...
            return (
                <div key={"ergopay"}>
                    <div mb="4">
                        ...
                        <p fontSize={"md"}>Scan the QR code below with an ErgoPay compatible wallet to sign the transaction.</p>
                    </div>
                    <div textAlign={"center"} mb="2">
                        <p fontSize={"sm"}>QR Code</p>
                    </div>
                    <div w={148} margin="auto" mb="4" overflow="hidden" borderRadius={"lg"} bgColor="white" p="10px">
                        <QRCode                      
                            size={128}
                            value={`ergopay://${skyHarborApiRoot}/api/ergopay/getTx/${txId}/${userAddress}`}/>
                    </div>
                    <div mb="4">
                        <p textAlign={"center"} fontSize="sm">or</p>
                    </div>
                    <div mb="4" spacing={2} columns={{base: 1, md: 2}} textAlign={"center"}>
                        <Button size={"lg"} onClick={copyErgoPay}>Copy request</Button>
                        <Button size={"lg"} colorScheme={"blue"} onClick={signErgoPay}>Open wallet</Button>
                    </div>
                ...
                <div key={"submitted"}>
                    <div maxW={300} mx="auto" my="6">
                        <div overflow={"hidden"} borderRadius={"lg"} position="relative">
                            <div textAlign={"center"} width="100%" position={"absolute"} zIndex={10} top="0" left="0" height={"100%"}>
                        ...
                        {box?.nft_name && <p fontWeight={"semibold"} mt="2" textAlign={"center"} noOfLines={1} width={"100%"}>{box.nft_name}</p>}
                    ...
                    <Button mb="3" isFullWidth colorScheme="green" variant={"outline"} onClick={()=>gotoTransaction(txId)}>View Tx on Explorer</Button>
                    <p mb="1" fontWeight={"semibold"} fontSize={"lg"}>Transaction ID: <p as="span"  color="green.400">{txId}</p></p>
                    <p fontSize={"sm"} color={"gray.400"}>Now we wait until it is confirmed on the blockchain. It should take about 2-10 minutes.</p>
